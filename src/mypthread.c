		    ...
		    int priority_A)
{
	...
	if (mythread_q_head == NULL) {
		...
		my_thread_create(&idle_u_tcb, NULL, mythread_idle, NULL, priority_A);
	}
	...
	if ((tid = clone(mythread_wrapper, (char *)child_stack, clone_flags, new_node)) == -1) {
		printf("clone failed! \n");
		printf("ERROR: %s \n", strerror(errno));
		return (-errno);
	}
...
void *mythread_idle(void *phony)
{
	...
	while (1) {
		...
		//puntero del TCB
		traverse_tcb = __mythread_ptr_act();
